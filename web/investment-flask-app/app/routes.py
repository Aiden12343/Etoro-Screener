...
def user_performance(username):
    try:
        user_heatmap_data, portfolio_data, risk_exposure_data, avatar_url, about_me = fetch_user_data(username)
        ...
        risk_dict = {risk['TickerName']: risk['riskContributionPct'] for risk in mapped_risk_exposure_data}
        for position in mapped_portfolio_data:
            position['riskContributionPct'] = risk_dict.get(position['TickerName'], 0)
        
        # Paginate portfolio data
        mapped_portfolio_data.sort(key=lambda x: x[sort_by], reverse=reverse)
        start = (page - 1) * per_page
        end = start + per_page
        paginated_portfolio_data = mapped_portfolio_data[start:end]
        has_more_portfolio_data = len(mapped_portfolio_data) > end

        next_order = 'desc' if sort_order == 'asc' else 'asc'

        return render_template('username.html', username=username, portfolio_data=paginated_portfolio_data, user_heatmap_data=user_heatmap_data, sort_by=sort_by, sort_order=sort_order, next_order=next_order, page=page, has_more_portfolio_data=has_more_portfolio_data, avatar_url=avatar_url, about_me=about_me)
